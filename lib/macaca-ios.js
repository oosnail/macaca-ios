...
IOS.prototype.startDevice = function *(caps) {
  ...
  yield _.retry(co.wrap(function *() {
    ...
    return yield that.xctest.start({
      ...
      case 1:
        if (app) {
          try {
            ...
            yield this.device.installUntilReady(app, bundleId);
...
IOS.prototype.startSimulator = function *() {
  ...
  if (reuse && isBooted) {
    ...
    if (reuse === 1 && app) {
      ...
      yield this.device.installUntilReady(app, this.bundleId);
    }
  ...
  // after device resume, should also based on app info and reuse to uninstall/install app
  if (reuse === 1 && app) {
    yield this.device.uninstall(this.bundleId);
    yield this.device.installUntilReady(app, this.bundleId);
  }
  ...
  for (var i = 0; i < value.length; i++) {
    ...
    if (keyEvent) {
      ...
      if (keyEvent === 'homescreen') {
        yield this.proxyCommand('/wd/hub/session/:sessionId/homeScreen', 'POST', {});
      }
    } else {
      arrText.push(key);
    }
